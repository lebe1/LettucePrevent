...
# ---------------------------------------------------------------------------
# The token cached by `huggingface-cli login` lives in ~/.cache/huggingface/.
# Even though that path is on /home (which compute nodes can read), some
# library versions don't pick it up reliably across node boundaries. Reading
# it ourselves and exporting HF_TOKEN guarantees the gated-model auth works.
...
# Modern transformers/huggingface_hub also read these:
