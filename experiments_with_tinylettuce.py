...
class DebugLogitsProcessor(LogitsProcessor):
    ...
    def __init__(self, tokenizer,  input_ids: torch.LongTensor):
        self.tokenizer = tokenizer
        self.detector = HallucinationDetector(
            method="transformer",
            model_path="KRLabsOrg/tinylettuce-ettin-68m-en",
        )
        self.context = self.tokenizer.decode(input_ids[0], skip_special_tokens=True)
        print("\n=== CONTEXT ===")
        print(self.context)
    ...
    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor) -> torch.FloatTensor:
        ...
        answer = generated_text[len(self.context):]
        print(answer)
        ...
        for rank, (tok, val) in enumerate(zip(top_tokens, top_vals), start=1):
            ...
            print("---------ANSWER+TOKEN------------")
            answer_with_token = answer + " " + tok[1:]
            print(answer_with_token)
            print(self.detector.predict(context=self.context, answer=answer_with_token, output_format="spans"))
...
debug_logits_processor = DebugLogitsProcessor(tokenizer, input_ids)
